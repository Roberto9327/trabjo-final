...
if(isset($_SESSION["nombre"]) && $_SESSION["AccesoSuperUser"] == 'Administrador')
{
	...
	if (!$_GET || $_GET['pagina'] <=0 ||$_GET['pagina'] > $paginas) {
		header('Location:ver_producto_new.php?pagina=1');
	}
...
<div id="contenidos">
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination">
			<li class="page-item <?php echo $_GET['pagina']<= 1? 'disabled' : '' ?>"><a class="page-link" href="ver_producto_new.php?pagina=<?php echo $_GET['pagina'] - 1?>">Anterior</a></li>


			<?php for ($i=0; $i < $paginas; $i++) {  ?>
				<li class="page-item <?php echo $_GET['pagina']==$i+1 ? 'active' : ''?>">
					<a class="page-link" href="ver_producto_new.php?pagina=<?php echo $i+1; ?>">
						<?php echo $i+1; ?>
					</a>
				</li>
			...
			<?php }
		      $paginas = $paginas - 1; ?>
		     <li class="page-item <?php echo $_GET['pagina']>= $paginas+1? 'disabled' : '' ?>"><a class="page-link" href="ver_producto_new.php?pagina=<?php if($_GET['pagina'] > $paginas){echo 1;}else{echo $_GET['pagina'] + 1;}  ?>">Siguiente</a></li>
		 </ul>
		</nav>
